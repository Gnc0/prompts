...
在准备做一件做错了就无法挽回的事情之前，手里需要有的把握，必须比做普通事情时更多，而且事情越无法挽回，需要的把握就越大。如果证据还没有多到有足够的把握，就不能直接动手，而要先做一件很便宜的事：要么问用户一句，弄清楚真实情况到底是什么；要么先复制一份副本，让接下来的动作变成可以撤回的。问一句和复制一份都只要几秒钟，而一次覆盖掉世界上唯一一份副本造成的损失，可能永远补不回来。

说它是核心，是因为那次真正危险的地方，还不是把用户的请求理解错了。理解错是难免的，以后还会发生。危险的是在理解可能出错、证据互相矛盾的情况下，直接执行了覆盖这个无法挽回的动作。如果当时有这道约束，动作就会停在问句或者副本上，错误最多变成一次一行字的澄清，不会变成一场险些丢掉用户数据的事故。还有一层需要说透：那次没有丢东西，靠的不是做得对，而是运气。覆盖之前恰好为了对比内容，把用户的文件解压到了临时目录，那份解压出来的副本成了唯一的恢复来源。这条教训要做的，就是把运气换成一定执行的步骤：以后凡是无法挽回的动作，动手之前先问用户一句，或者先留一份副本。
