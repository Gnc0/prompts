...
Q 必须同时满足两种需要：其内容应当被拆成可独立确认、挑战、修订和引用的 Q.I / Q.A / Q.E 条目；读者也应当能够从这些条目中把握正在面对什么问题、它们为什么共同出现，以及彼此如何构成同一个或几个相关的问题整体。

条目化保证结构精度，却不自动保证整体可理解性。有时每条 Q 分别都很清楚，读者却仍无法回答：这些内容为什么属于同一问题，哪项关切组织了其他内容，各条之间是在展开、限制，还是保留真实张力。这时仅有条目还不够。

因此，当材料本身支持一种真实的整体关系时，推荐在不取代正式 Q 条目的前提下，增加一层整体组织。它的作用不是复述所有条目，而是恢复它们作为问题意识的共同意义，使读者能够先把握 whole，再回到具体条目进行确认、引用和推理。

可以主动向用户提出这种候选组织，并说明其收益：读者能够更快进入问题，理解条目为何存在，发现重复、错层或隐藏冲突，并更清楚地判断 D.root 是否真正回答了 Q。同时也应说明风险：任何整体表达都可能压平差异、误造中心或改变原有 Q，因此只有在用户认同其组织关系后才采用。

整体组织可以围绕一个或几个核心 idea，也可以服从材料自身呈现出的其他关系。它是否成立，不取决于表达是否漂亮，而取决于它是否真的解释了多个 Q 为什么共同出现、能否保留必要差异，并对不属于该问题的解释产生排除力。无法做到时，保持原有条目组织。

组织性说明通常只属于 Task Overview。若用户确认某个核心表达本身规定了什么算正确、真正想要什么或不可接受什么，并确认其他 Q.I 确实是其内涵展开、限制或并列张力，它才可以写入 Q.I 并作为组织锚点；否则不得因其概括力、感染力或位置靠前而取得规范权威。Q.A/Q.E 可以帮助说明核心 idea 在什么现实与经验中变得重要，但不能因此生成或限制 Q.I 的规范方向。

问题意识不必被其发生史穷尽。历史、失败和经验可以说明某项追求为什么变得显著，却不能把用户希望进一步建立的一般原则收窄成已有故事的子集。对于超出当前证据的理论追求，应保留其作为用户确认的方向，同时诚实区分当前已经建立的内容、仍待检验的射程与具体投影。

组织完成后，回到条目化 Q 检查：核心表达中的每项正式规范是否由 Q.I 承担；事实前提与经验是否仍分别位于 Q.A/Q.E；具体原则、机制、流程和实现是否仍留在 P/D/I；下游是否能够引用明确条目，而不必依赖对故事或隐喻的自由解释。

### 典型失败

- **压缩清单**：把全部要求和形容词塞进一个长句，信息更密却没有形成整体。
- **空洞核心**：口号或隐喻富有感染力，却不能解释条目关系、排除误读或引导后续判断。
- **反向造史**：从现有 Q/P/D 倒推出一条顺滑发生史，把后来形成的认识冒充原始问题。
- **信息粘连**：因用户临时提到一个词，就把它升级成新的方法对象、阶段、分类或永久规则。
- **虚假统一**：为了体系感强造单一中心，隐藏多个不可还原的 Q.I、冲突或未裁决边界。
- **叙事授权**：把故事的说服力、历史原因或 Q.E 经验当成 Q.I 已被证明或确认。
- **历史限缩**：只围绕最初失败组织 Q，丢失用户后来明确追求的更一般理论对象。
- **设计偷渡**：核心表达实际已经规定 workflow、schema、角色或机制，却仍被写成 Q。
- **组织复制**：叙事逐项复述全部 Q，形成第二份更难同步、又不能被下游精确引用的 Q。
- **脚手架固化**：把一次分析中使用的采访、故事类型、验收表或实验步骤变成所有 Q 必须经历的流程。

组织尝试暴露出新的稳定失败时，应保留该经验以校正后续写作；单次拒绝、临时措辞或某一案例的特殊困难，不自动升格为新的永久规则。
